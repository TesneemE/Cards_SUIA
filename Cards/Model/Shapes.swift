...
//  Cards
//
//  Created by Tes Essa on 11/1/24.
//
...
//Shape is associated type, not existential type protocol like CardElement with cards that can be found w/ int ID
//constrained to a type, so can't set array of it
//shape can be rectangle, triangle diff types

struct Shapes_Previews: PreviewProvider {
    static let currentShape = Lens()
    static var previews: some View {
        currentShape
//            .stroke(lineWidth: 5) //only used on objects conforming to shape, so first one
//            .stroke(style: StrokeStyle(dash: [30, 10])) animation
            .stroke(
              Color.primary,
...
struct Cone: Shape {
  func path(in rect: CGRect) -> Path {
    var path = Path()
      ...
      path.addArc(
        center: CGPoint(x: rect.midX, y: rect.midY),radius: radius,
